    ...
    <div class="relative shadow-md sm:rounded-lg mt-5">
        <div class="flex justify-between items-center flex-wrap p-4 bg-sidebar rounded-t-3xl border-b border-zinc-200">
            ...
            <div class="desktopFilters flex justify-start items-center flex-wrap gap-3 mt-3 sm:mt-0 md:mt-0">
                ...
                <button @click="$emit('openDate', '')" v-if="groupMembers != ''"
        ...
        <div class="themeOverflowCustom themeOverflowTable">
            <table class="theme-table w-full text-sm text-left text-gray-500 dark:text-gray-400 themeTableGroupMember">
                <thead class="text-xs text-gray-700 uppercase bg-sidebar dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="text-neutral-300 font-semibold text-base p-4">{{ trans("lang.labels.name") }}
                        </th>
                        <th scope="col" class="text-neutral-300 font-semibold text-base p-4">{{ trans("lang.labels.email")
                        }}</th>
                ...
                <tbody>
                    <tr v-if="groupMembers.length == 0" class="themeNoFound">
                        <td colspan="5">
                    ...
                    <tr v-for="member in groupMembers" :key="member"
                        ...
                        <td class="p-4">
                            <span v-if="member.user_archived_at != null">{{ trans("lang.modal.archive") }}</span>
                            <img :src="mediaPath + member.avatar" alt='' class="w-14 h-14 rounded-lg object-cover">
                        </td>
                        ...
                        <td class="p-4">
                            <button v-if="member.scheduled_at == null" @click="$emit('openDate', member.user_id)" class="px-3 py-2 rounded-lg text-sm text-btnSubmitText bg-btnSubmitBg 
                                            hover:bg-amber-500 hover:text-white">{{ trans("lang.labels.schedule_date")
                                            }}</button>
                            <span v-else>{{ member.scheduled_at }}</span>
                        </td>
...
import {
    EllipsisVerticalIcon, PencilSquareIcon, TrashIcon, FunnelIcon
...
export default {
    ...
    methods: {
        ...
        createTable(page) {
            ...
            axios.get('/api/group-members/' + this.id, { params: this.queryData })
                .then((response) => {
