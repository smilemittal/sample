<template>
    <div class="relative shadow-md sm:rounded-lg mt-7">
        ...
        <div class="themeOverflowCustom themeOverflowTable">
            <table class="theme-table w-full text-sm text-left text-gray-500 dark:text-gray-400 themeTableGroupModule">
                <thead class="text-xs text-gray-700 uppercase bg-sidebar dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="text-neutral-300 font-semibold text-base p-4">{{
                            trans("lang.labels.schedule") }}</th>
                        <th scope="col" class="text-neutral-300 font-semibold text-base p-4">{{
                            trans("lang.labels.closed_on") }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="groupModules.length == 0" class="themeNoFound">
                        <td colspan="5">
                    ...
                    <tr v-for="modules in groupModules" :key="modules.id"
                        ...
                        <td class="p-4">
                            ...
                            <div class="btn-group dropstart">
                                ...
                                <div class="dropdown-menu theme-dropdown-menu bg-card rounded-2xl p-0.5"
                                    v-if="modules.archived_at == null && modules.company_archived_at == null">
                                    <ul class="py-0">
                                        <li class="px-4 py-3 bg-card text-white rounded-t-2xl">{{
                                            trans("lang.labels.options") }}</li>
                                        <li class="">
                                            <a @click="openAssign(modules.formId)"
                                                ...
                                                {{ trans("lang.labels.reassign_settings") }}
                                            </a>
                                        </li>
                                        <li v-if="modules.is_reassign" class="">
                                            <a @click="removeAssign(modules.formId)"
                                                class="cursor-pointer flex items-center bg-body w-full px-4 py-3 hover:bg-card text-white">
                                                <PencilIcon class="h-4 w-4 text-white mr-3" aria-hidden="true" />
                                                {{ trans("lang.labels.remove_reassign_settings") }}
                                            </a>
                                        </li>
                                        <li class="">
                                            <a v-if="modules.archived_at != null || modules.company_archived_at != null"
                                                class="cursor-pointer flex items-center bg-body w-full px-4 py-3 hover:bg-card text-red-600">
                                                <TrashIcon class="h-4 w-4 text-red-600 mr-3" aria-hidden="true" />
                                                {{ trans("lang.labels.remove_module") }}
                                            </a>
                                            <a v-else @click="$emit('openDeleteModule', modules.formId)"
                                                class="cursor-pointer flex items-center bg-body w-full px-4 py-3 hover:bg-card text-red-600">
                                                <TrashIcon class="h-4 w-4 text-red-600 mr-3" aria-hidden="true" />
                                                {{ trans("lang.labels.remove_module") }}
                                            </a>
                        ...
                        <td class="p-4">
                            ...
                            <img :src="mediaPath + modules.image" alt='' class="mt-2 w-14 h-14 rounded-lg object-cover">
                        </td>
                        <td class="p-4">
                            {{ modules.archived_at }} {{ modules.company_archived_at }}
                            <span class="breakWord themeManageTitle">{{ modules.display_title }}</span>
                        </td>
                        <td class="p-4">
                            <span>{{ modules.scheduleTime }}</span>
                        </td>
                        <td class="p-4">
                            <span>{{ modules.closedOn }}</span>
                        </td>
                    </tr>
                </tbody>
            </table>
...
import {
    EllipsisVerticalIcon, PencilIcon, EyeIcon, TrashIcon,
    FunnelIcon
...
export default {
    ...
    methods: {
        ...
        createTable(page) {
            ...
            axios.get('/api/group-modules/' + this.id, { params: this.queryData })
                .then((response) => {
                    if (response.status == 200) {
                        this.groupModules = response.data.data;
        ...
        openAssign(id) {
            this.$emit('openAssign', id);
        },
        removeAssign(id) {
            this.$emit('openAssignSetting', id);
        }
    },
    created: function () {
        this.createTable(1);
