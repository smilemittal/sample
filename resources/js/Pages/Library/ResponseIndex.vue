    ...
    <AuthenticatedLayout>
      <div class="mb-4">
        <div class="page-header">
          <h4 class="text-3xl text-neutral-300 font-bold">Responses</h4>
        </div>
      </div>
      <div>
        <div class="drop-shadow-lg h-full drop-shadow-md rounded-3xl mt-5">
          ...
          <div class="themeOverflowTable">
            <div class="flex p-3 bg-cardtop" v-for="response in responseData" :key="response">
              <div>
                <StopCircleIcon class="h-8 w-8 text-amber-500" aria-hidden="true" />
              </div>
              <div class="ml-4">
...
export default {
  ...
  methods: {
    ...
    createTable(page) {
      ...
      axios
        .get('api/form/responses', { params: this.queryData })
        .then((response) => {
